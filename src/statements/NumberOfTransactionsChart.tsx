...
import { StatementsDashboardTableProps } from './StatementsDashboardTable';
    ...
    return accountMap.map((account, stmts) => {
        return {
            ...
            data: xAxisLabels.map((label) => {
                ...
                else return matchingStatement.numTransactions
            }),
            valueFormatter: ((val) => {
                if (val) return val
                else return `MISSING`}
...
interface NumberOfTransactionsChartProps {
    statements: BankStatementInfo[]
}

export default function NumberOfTransactionsChart({statements}: NumberOfTransactionsChartProps) {
  ...
  const totalNumberOfTransactions = statements.reduce(((prev, stmt) => {return prev + stmt.numTransactions}), 0)

  return (
    <Card variant="outlined" sx={{ width: '100%' }}>
      <CardContent>
        <Typography component="h2" variant="subtitle2" gutterBottom>
          Number of Transactions By Statement Month
        </Typography>
        <Stack sx={{ justifyContent: 'space-between' }}>
          ...
          >
            ...
            |
            <Typography variant="h4" component="p">
              {Math.round(totalNumberOfTransactions/statements.length)}
            </Typography>
